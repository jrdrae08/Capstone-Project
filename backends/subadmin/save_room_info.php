...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
  ...
  if ($businessNameResult) {
    ...
    for ($i = 1; $i <= 6; $i++) {
      ...
      if (isset($_FILES[$imageKey]) && $_FILES[$imageKey]['error'] == 0) {
        ...
        if (!in_array($imageFileType, $allowedTypes)) {
          $_SESSION['errors'][] = ['field' => $imageKey, 'message' => "Invalid file type: " . $_FILES[$imageKey]['name']];
          continue;
        }

        // Validate file size (e.g., limit to 5MB)
        if ($_FILES[$imageKey]['size'] > 5 * 1024 * 1024) {
          $_SESSION['errors'][] = ['field' => $imageKey, 'message' => "File size exceeds the limit of 5MB: " . $_FILES[$imageKey]['name']];
          continue;
        }
        ...
        // Resize image
        $resizedImagePath = resizeImage($_FILES[$imageKey]['tmp_name'], $targetFilePath, 1920, 1080);

        if ($resizedImagePath) {
          $images[$imageKey] = $resizedImagePath;
        } else {
          $_SESSION['errors'][] = ['field' => $imageKey, 'message' => "Failed to resize or move uploaded file: " . $_FILES[$imageKey]['name']];
          $_SESSION['form_data'] = $_POST;
          header('Location: ../../businessowner/add-rooms.php');
          exit();
        }
    ...
    header('Location: ../../businessowner/add-rooms.php?success=true');
    exit();
  } else {
